<?php
   ...
   class Message
   {
      ...
      function setMessageID($id = null)
      {
          ...
          {
              $this->msg_id  = isset($this->msg_id) ? $this->msg_id : NULL;	
          }
      ...
      function getMessages($uid = null, $lastDate = null)
      {
         ...
         $stmt = "SELECT $fields FROM $this->msg_tbl WHERE MSG_DATE <= $lastDate ORDER BY MSG_DATE DESC";
      ...
      {
         $fields = implode(',', array_keys($this->fields));
         $stmt = "SELECT $fields FROM $this->msg_tbl";
